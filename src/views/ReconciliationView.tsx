...
export default function ReconciliationView({ transactions, setTransactions, pushToast, onRefresh }: ReconciliationViewProps) {
  ...
  return <div className="space-y-6">
    ...
    <div className="card"><div className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-b border-ink-200"><div><h3 className="font-bold text-ink-900">Pagos recibidos desde la app móvil</h3><p className="text-sm text-ink-500 mt-1">Los comprobantes ya llegan con revisión OCR e IA; aquí se validan contra el extracto.</p></div><button onClick={() => void onRefresh()} className="btn-secondary"><RefreshCw className="h-4 w-4" /> Actualizar</button></div><div className="flex gap-1 p-3 border-b border-ink-100 overflow-x-auto">{(['all', 'QR', 'DEPOSITO', 'EFECTIVO', 'TRANSFERENCIA'] as ChannelTab[]).map((tab) => <button key={tab} onClick={() => setActiveTab(tab)} className={`btn ${activeTab === tab ? 'bg-navy-800 text-white' : 'text-ink-500 hover:bg-ink-100'}`}>{tab === 'all' ? <Landmark className="h-4 w-4" /> : (() => { const Icon = channelIcon(tab); return <Icon className="h-4 w-4" />; })()}{tab === 'all' ? 'Todos' : CHANNEL_LABELS[tab]}</button>)}</div><div className="overflow-x-auto"><table className="w-full"><thead><tr><th className="table-head">Estado</th><th className="table-head">N° Transacción</th><th className="table-head">Estudiante</th><th className="table-head">Canal</th><th className="table-head">Concepto</th><th className="table-head text-right">Monto</th><th className="table-head">Fecha</th><th className="table-head text-right">Acciones</th></tr></thead><tbody className="divide-y divide-ink-100">{filtered.map((tx) => { const Icon = channelIcon(tx.canal); return <tr key={tx.id} className="hover:bg-ink-50"><td className="table-cell">{statusBadge(tx.estado)}</td><td className="table-cell font-mono text-xs font-semibold">{tx.numeroTransaccion}</td><td className="table-cell"><b>{tx.estudiante}</b><div className="text-xs text-ink-400">{tx.ci}</div></td><td className="table-cell"><span className="badge-navy"><Icon className="h-3 w-3" />{CHANNEL_LABELS[tx.canal]}</span></td><td className="table-cell">{tx.concepto}</td><td className="table-cell text-right font-semibold">Bs {formatBs(tx.monto)}</td><td className="table-cell text-xs text-ink-500">{formatDateTime(tx.fecha)}</td><td className="table-cell"><div className="flex justify-end gap-1"><button onClick={() => setSelected(tx)} className="btn-ghost text-xs"><Eye className="h-4 w-4" /> Ver comprobante</button>{tx.estado !== 'CONCILIADO' && <button onClick={() => void changeStatus(tx, 'CONCILIADO')} className="btn-success text-xs"><CheckCircle2 className="h-4 w-4" /> Conciliar</button>}</div></td></tr>; })}</tbody></table></div></div>
    {selected && <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}><div className="absolute inset-0 bg-navy-950/40 backdrop-blur-sm" /><div className="relative w-full max-w-lg rounded-2xl bg-white shadow-pop p-6" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between mb-4"><div><h3 className="font-bold text-ink-900">{DOCUMENT_LABELS[selected.canal]}</h3><p className="text-xs text-ink-500">{selected.numeroTransaccion} · Validación OCR: {selected.ocrData?.es_valido ? 'completada' : 'pendiente'}</p></div><button onClick={() => setSelected(null)} className="btn-ghost"><X className="h-5 w-5" /></button></div><div className="rounded-xl border border-ink-200 bg-ink-50 p-6 font-mono text-sm space-y-2"><p className="font-bold">Registro bancario</p><p>Número: {selected.numeroTransaccion}</p><p>Estudiante: {selected.estudiante}</p><p>Concepto: {selected.concepto}</p><p>Fecha: {formatDateTime(selected.fecha)}</p><p className="text-lg font-bold text-navy-800">Monto: Bs {formatBs(selected.monto)}</p>{selected.observacion && <p className="text-xs text-red-600">Obs: {selected.observacion}</p>}{selected.ocrData && <p className="text-xs text-emerald-700">Datos extraídos por OCR disponibles en el registro.</p>}</div><div className="flex gap-2 mt-5">{selected.estado !== 'CONCILIADO' && <button onClick={() => { void changeStatus(selected, 'CONCILIADO'); setSelected(null); }} className="btn-success flex-1"><CheckCircle2 className="h-4 w-4" /> Conciliado</button>}{selected.estado !== 'RECHAZADO' && <button onClick={() => { void changeStatus(selected, 'RECHAZADO'); setSelected(null); }} className="btn-danger flex-1"><XCircle className="h-4 w-4" /> Rechazar</button>}<button onClick={() => setSelected(null)} className="btn-secondary">Cerrar</button></div></div></div>}
  </div>;
}
