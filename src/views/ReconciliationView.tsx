...
import { UploadCloud, FileSpreadsheet, CheckCircle2, AlertTriangle, XCircle, Eye, ShieldAlert, Landmark, QrCode, ArrowRightLeft, RefreshCw, X, Calendar, Filter, Loader2 } from 'lucide-react';
...
// Canal excluye EFECTIVO para centrarse únicamente en la conciliación bancaria
type ChannelTab = 'all' | 'QR' | 'DEPOSITO' | 'TRANSFERENCIA';
type StatusTab = 'all' | 'CONCILIADO' | 'PENDIENTE' | 'RECHAZADO' | 'REVISION';

const CHANNEL_LABELS: Record<Exclude<PaymentChannel, 'EFECTIVO'>, string> = { 
  QR: 'QR', 
  DEPOSITO: 'Depósito', 
  TRANSFERENCIA: 'Transferencia' 
};

const DOCUMENT_LABELS: Record<Exclude<PaymentChannel, 'EFECTIVO'>, string> = { 
  QR: 'comprobante QR', 
  DEPOSITO: 'voucher de depósito', 
  TRANSFERENCIA: 'comprobante interbancario' 
};
...
export default function ReconciliationView({ transactions, setTransactions, pushToast, onRefresh }: ReconciliationViewProps) {
  ...
  // Filtrado avanzado: Oculta por defecto los pagos en efectivo y filtra por canal bancario, estado y fechas
  const filtered = useMemo(() => {
    return transactions.filter((item) => {
      // Excluir pagos de tipo EFECTIVO de la vista de conciliación bancaria
      if (item.canal === 'EFECTIVO') return false;
  ...
  const counts = {
    conciliado: transactions.filter((item) => item.canal !== 'EFECTIVO' && item.estado === 'CONCILIADO').length,
    pendiente: transactions.filter((item) => item.canal !== 'EFECTIVO' && item.estado === 'PENDIENTE').length,
    rechazado: transactions.filter((item) => item.canal !== 'EFECTIVO' && item.estado === 'RECHAZADO').length,
  };
  ...
  const channelIcon = (channel: PaymentChannel) => 
    channel === 'QR' ? QrCode : 
    channel === 'TRANSFERENCIA' ? ArrowRightLeft : Landmark;

  const statusBadge = (status: ReconStatus) => 
    status === 'CONCILIADO' ? <span className="badge-green">✅ Conciliado</span> : 
    status === 'RECHAZADO' ? <span className="badge-red">❌ Rechazado</span> : 
    status === 'REVISION' ? <span className="badge-yellow">⚠️ Revisión</span> : 
    ...
    <div className="space-y-6">
          ...
          <button onClick={() => inputRef.current?.click()} disabled={uploading} className="mt-5 w-full rounded-xl border-2 border-dashed border-ink-300 hover:border-navy-500 hover:bg-navy-50 p-8 transition text-center disabled:opacity-60 flex flex-col items-center justify-center">
            {uploading ? (
              ...
              <>
                ...
                <p className="text-xs text-ink-500 mt-1">El sistema evaluará únicamente los pagos bancarios pendientes.</p>
              </>
            )}
      ...
      <div className="card">
        <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-b border-ink-200">
          <div><h3 className="font-bold text-ink-900">Transacciones bancarias ({filtered.length})</h3><p className="text-sm text-ink-500 mt-1">Revisa los comprobantes, estados y observaciones de canales QR, Depósito y Transferencia.</p></div>
          <button onClick={() => void onRefresh()} className="btn-secondary"><RefreshCw className="h-4 w-4" /> Actualizar</button>
        </div>
        
        {/* Pestañas de canales (Sin efectivo) */}
        <div className="flex gap-1 p-3 border-b border-ink-100 overflow-x-auto">
          {(['all', 'QR', 'DEPOSITO', 'TRANSFERENCIA'] as ChannelTab[]).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`btn text-xs ${activeTab === tab ? 'bg-navy-800 text-white' : 'text-ink-500 hover:bg-ink-100'}`}>
              {tab === 'all' ? <Landmark className="h-4 w-4" /> : (() => { const Icon = channelIcon(tab as PaymentChannel); return <Icon className="h-4 w-4" />; })()}
              {tab === 'all' ? 'Todos los canales' : CHANNEL_LABELS[tab as Exclude<PaymentChannel, 'EFECTIVO'>]}
            </button>
          ))}
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-ink-100">
              {filtered.length === 0 ? (
                <tr><td colSpan={8} className="text-center py-8 text-ink-400 text-sm">No se encontraron transacciones bancarias.</td></tr>
              ) : (
                filtered.map((tx) => { 
                    ...
                    <tr key={tx.id} className="hover:bg-ink-50">
                      ...
                      <td className="table-cell">
                        <span className="badge-navy">
                          ...
                          {CHANNEL_LABELS[tx.canal as Exclude<PaymentChannel, 'EFECTIVO'>] || tx.canal}
                        </span>
                      </td>
                      <td className="table-cell">
                        <div>{tx.concepto}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          ...
          <div className="relative w-full max-w-lg rounded-2xl bg-white shadow-pop p-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="font-bold text-ink-900">{DOCUMENT_LABELS[selected.canal as Exclude<PaymentChannel, 'EFECTIVO'>] || 'comprobante bancario'}</h3>
